...
# A directory for outputting figures
figures <- "./figures"

## A vector of all packages used in this script
all_packages <- c("tdar", "treecats", # Packages for integration analyses
                  "magrittr", "tidyverse", # Packages for tidy code
                  "RColorBrewer", "htmlwidgets", "htmltools", "plotly", "DT",
                  "bibtex", "knitcitations", "kableExtra", "psych") # Plotting and rmarkdown

# load required packages, installing as needed
if(!require("devtools", quietly = TRUE))
  install.packages("devtools")
if(!require("tdar", quietly = TRUE))
  devtools::install_github("bocinsky/tdar")
if(!require("treecats", quietly = TRUE))
  devtools::install_github("bocinsky/treecats")
suppressWarnings(
  purrr::walk(all_packages, 
              function(p){
                if(!require(p, character.only = TRUE)){
                  devtools::install_cran(p, character.only = TRUE)
...
plot_summary <- function(x){
  (x %>% 
     ...
     ggplot(mapping = ggplot2::aes(x = Dataset %>%
...
integration.data <- 
  ...
  dplyr::mutate(count = as.character(`count column`),
                count = if_else(is.na(count) | count == ".", "1.0", count),
  ...
  dplyr::select(-`count`,
                -`count column`) %T>%
  readr::write_csv("../data/derived_data/tdar-integration-SWTP_raw.csv") %>%
...
bone_weight_plot <- function(x){
  (x %>%
     ...
     ggplot2::geom_pointrange(aes(ymin = `Weight (IQR—lower)`,
                                  ...
                                  text = paste0(Dataset,"\n",
                                                ...
                                                "Mean bone weight: ", `Weight (mean)` %>%
                                                  round(digits = 3),"\n"))) +
     ggplot2::geom_point(aes(y = `Weight (mean)`,
                             text = paste0(Dataset,"\n",
                                           "Median bone weight: ", `Weight (median)` %>%
                                             round(digits = 3),"\n",
                                           "Mean bone weight: ", `Weight (mean)` %>%
...
art_phalanxes <- integration.data %>%
  dplyr::filter(Taxon %in% c("Artiodactyla"),
                Element %in% (integration.data$Element %>% tct_get_descendents("3rd Phalanx"))) %>%
  var_summary(Completeness) %>%
  affected_summary(`<25%`) %>%
...
art_elements$Element[art_elements$Element %in% (integration.data$Element %>% tct_get_descendents("Pelvic Girdle"))] <- "Pelvis"
...
tax_summary <- function(x, taxa, var){
  ...
  x %>%
    ...
    dplyr::mutate(Taxon = tct_drop(Taxon)) %>%
    ...
    tidyr::spread(!!quo_var,
                  n,
                  drop = FALSE) %>%
    dplyr::mutate_all(.funs = function(x){ifelse(is.na(x),
                                                 0,
                                                 x)}) %>%
    dplyr::ungroup()
}
  ...
  x %>%
    dplyr::mutate(
      ...
      `n affected` = rowSums(dplyr::select(.,
        ...
        as.integer(),
      `p affected` = `n affected`/`n recorded`,
      `p affected CI lower` = map2_dbl(`n affected`,
                                       `n recorded`,
                                       function(x,y){ifelse(y > 0, 
                                                            binom.test(x,y)$conf.int[[1]],
                                                            NA)
                                       }),
      `p affected CI upper` = map2_dbl(`n affected`,
                                       `n recorded`,
                                       function(x,y){ifelse(y > 0, 
                                                            binom.test(x,y)$conf.int[[2]],
                                                            NA)
...
tax_plotter <- function(x){
  (x %>% 
     ...
     ggplot2::geom_pointrange(aes(ymin = `p affected CI lower`,
                                  ymax = `p affected CI upper`,
                                  text = paste0(Dataset,"\n",
                                                Taxon,"\n",
                                                "Prop. affected: ", `p affected` %>% 
...
lag_elements$Element[lag_elements$Element %in% (integration.data$Element %>% tct_get_descendents("Pelvic Girdle"))] <- "Pelvis"
