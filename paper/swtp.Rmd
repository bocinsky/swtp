...
```{r collapse-taxa}
integration.data %<>%
  dplyr::mutate(`Proximal/distal` = `Proximal/distal` %>%
...
Table \@ref(tab:data-overview) summarizes the data sets, including the counts of samples with valid measurements across each variable reviewed in the Southwest Taphonomic Protocol. Our final dataset includes a total NISP of `r sum(data_overview[["Total NISP"]])` across all taxa, including `r sum(data_overview[["Art. NISP"]])` artiodactyl specimens, `r sum(data_overview[["Lag. NISP"]])` lagomorph specimens, and `r sum(data_overview[["Meg. NISP"]])` turkey or large bird specimens.
...
# integration.data %>%
#   dplyr::filter(Dataset == "Pueblo Blanco") %>%
#   dplyr::group_by(Taxon) %>%
#   dplyr::summarise(n= n())
#   as.data.frame()
...
art_elements %<>%
  ...
  dplyr::mutate(`Proximal/distal` = "Proximal",
                `Proximal/distal` = factor(`Proximal/distal`, 
                                           levels = levels(art_elements$`Proximal/distal`),
                                           ordered = TRUE)) %>%
  ...
  dplyr::bind_rows(art_elements %>%
                     ...
                     dplyr::mutate(`Proximal/distal` = factor(`Proximal/distal`, 
                                           levels = levels(art_elements$`Proximal/distal`),
                                           ordered = TRUE))) %>%
  dplyr::bind_rows(art_elements %>%
...
art_survivorship <- art_elements %>%
  dplyr::left_join(readr::read_csv("./data/raw_data/Artiodactyl_density.csv",
                                   col_types = readr::cols(
                                     ...
                                     `Proximal/distal` = readr::col_character(),
...
lag_elements %<>%
  ...
  dplyr::mutate(`Proximal/distal` = "Proximal",
                `Proximal/distal` = factor(`Proximal/distal`, 
                                           levels = levels(lag_elements$`Proximal/distal`),
                                           ordered = TRUE)) %>%
...
meg_elements %<>%
  ...
  dplyr::mutate(`Proximal/distal` = "Proximal",
                `Proximal/distal` = factor(`Proximal/distal`, 
                                           levels = levels(meg_elements$`Proximal/distal`),
                                           ordered = TRUE)) %>%
