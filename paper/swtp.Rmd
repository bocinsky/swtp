...
art_survivorship <- art_elements %>%
  dplyr::left_join(readr::read_csv("./data/raw_data/Artiodactyl_density.csv",
                   ...
                   by = c("Element", 
                          "Proximal/distal")) %>%
  dplyr::group_by(Dataset) %>%
  dplyr::summarize(`Spearman's rho` = cor.test(n, 
                                               Density,
                                               method = "spearman",
                                               continuity = TRUE,
...
meg_survivorship <- meg_elements %>%
  dplyr::left_join(readr::read_csv("./data/raw_data/Meleagris_density.csv",
                   ...
                   by = c("Element", 
                          "Proximal/distal")) %>%
  dplyr::arrange(Dataset, Element, `Proximal/distal`) %>%
  dplyr::group_by(Dataset) %>%
  dplyr::summarize(`Spearman's rho` = cor.test(n, 
                                               Density,
                                               method = "spearman",
